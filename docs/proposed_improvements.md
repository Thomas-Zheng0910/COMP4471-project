...
```bash
# Launch all 6 experiments on GPUs 0-5:
...
# Single experiment (e.g., E4 dual-channel):
CUDA_VISIBLE_DEVICES=0 python -m train.train_depth \
  --cuda 0 --run_name E4_dual_channel \
  --use_edge_head true --edge_loss_weight 0.5 \
  --use_seg_auxiliary true --seg_loss_weight 0.5 \
  --seg_labels_path datasets/nyuv2_yolo_seg_labels.npy \
  --datasets nyuv2,tom --color_jitter 0.3 --deep_supervision true \
  --amp --epochs 20 --batch_size 2 --accum_steps 8 \
  --encoder_name dinov3_vitl16 --image_shape 384 384 \
  --hidden_dim 512 --depths 3 2 1
```
