...
NOTE: removed distributed training
"""
...
from model.unidepthv1.decoder import Decoder
...
from utils.misc import (get_params, match_gt, match_intrinsics,
                                 profile_method)
...
class UniDepthV1(
    ...
    def build(self, config):
        mod = importlib.import_module("model.encoder")
    ...
    def build_losses(self, config):
        ...
        for loss_name, loss_config in config["training"].get("losses", {}).items():
            mod = importlib.import_module("model.ops.losses")
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "training": {
            "losses": {
                "depth":      { "name": "SILog", "weight": 1.0, "output_fn": "sqrt", "input_fn": "log", "dims": [-2,-1], "integrated": 0.15 },
                "invariance": { "name": "SelfDistill", "weight": 0.1, "output_fn": "sqrt" },
                "camera":     { "name": "Regression", "weight": 0.25, "gamma": 1.0, "alpha": 1.0, "fn": "l2", "output_fn": "sqrt", "input_fn": "linear" },
            }
        },
    }
    ...
    H, W = 384, 384
    # ------------------------------------------------------------------ #
    # Use B=2: one "original" sample and one "flipped" counterpart.       #
    # SelfDistill internally calls input.chunk(n_flipped) to split the    #
    # batch into original vs flipped halves — it requires n_flipped >= 1. #
    # The standard training convention is:                                 #
    #   first  B//2 samples → originals  (flip=False)                     #
    #   last   B//2 samples → flipped    (flip=True)                      #
    # ------------------------------------------------------------------ #
    B = 2
    ...
    class DummyCamera:
        def __init__(self, B, H, W):
            ...
            # 3×3 intrinsics matrix required by SelfDistill / invariance loss
            self.K = torch.eye(3).unsqueeze(0).expand(B, -1, -1).clone()

        def get_rays(self, shapes):
            # shapes = (B, H, W) as passed by encode_decode
            b, h, w = shapes
            # Must return (B, 3, H, W) — the "b c h w" layout encode_decode expects
            return torch.rand(b, 3, h, w)

    dummy_camera = DummyCamera(B, H, W)

    dummy_inputs = {
        "image":      torch.rand(B, 3, H, W),
        "camera":     dummy_camera,
        "depth":      torch.rand(B, 1, H, W).clamp(min=0.01),  # positive depths for SILog
        "depth_mask": torch.ones(B, 1, H, W, dtype=torch.bool),
    }

    # First sample is the original; second is its horizontal flip.
    # SelfDistill counts flips via sum(flip==True), so exactly 1 flip
    # is needed when B=2.
    dummy_image_metas = [
        {"si": False, "flip": False},   # original
        {"si": False, "flip": True},    # flipped counterpart
    ]

    # Instantiate the model
    model = UniDepthV1(config)
    model.train()   # ensure forward_train path is taken

    # Run a forward pass
    outputs, losses = model.forward_train(dummy_inputs, dummy_image_metas)
    ...
    for key, value in outputs.items():
        if hasattr(value, "shape"):
            ...
            print(f"  {key}: {value}")
    print("\nLosses:")
    for key, value in losses["opt"].items():
        print(f"  {key}: {value.item():.6f}")

    # NOTE:
    # The data pipeline deliberately constructs paired batches of size 2N:
    # batch[0 : N]   → original images     (flip=False)
    # batch[N : 2N]  → their h-flipped twins (flip=True)
    # SelfDistill.forward then calls:
    # originals, flipped = input.chunk(n_flipped)  # splits on the flip count